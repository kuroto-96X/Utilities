...
### 4. タップボタン リップルエフェクト + 押下アニメーション（iOS Safari 対応）

iOS Safari では CSS `:active` がタッチで発火しないため、Blazor の Pointer Events で押下状態を管理する。  
`@onpointerdown` / `@onpointerup` / `@onpointerleave` は iOS Safari を含む全モダンブラウザで動作する。
...
CSS（既存の `.tap-button:active` は削除し、以下を追加）:
```css
.tap-button {
    position: relative;
    overflow: hidden;
    /* 既存スタイルはそのまま維持。:active は削除する */
}

/* iOS Safari 対応: :active の代わりに Blazor 管理の .pressed クラスを使用 */
.tap-button.pressed {
